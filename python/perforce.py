...
class P4Repo:
    ...
    def _flush_to_stream_and_changelist(
        self, current_client, prev_client_stream, prev_client_changelist
    ):
        """
        ...
        if stream_switch:
            self.perforce.logger.info(
                "previous client stream %s does not match %s, switching stream temporarily to flush"
                % (prev_client_stream, self.stream)
            )
                ...
                if self.client_type == "writeable":
                    ...
                    with open(bless_version_file, 'r') as file:
                        ...
                        if len(blessed_stream_and_version) == 2:
                            self.perforce.logger.warning(
                                "flushing workspace to previously blessed version: stream %s at changelist %s"
                                % (
                                    blessed_stream_and_version[0],
                                    blessed_stream_and_version[1],
                                )
                            )
                            ...
                            self.perforce.logger.warning(
                                "invalid bless.version format: %s. Expected format is stream@CL, e.g. //depot/main@123456"
                                % blessed_version_string
                            )

                if need_full_clean:
                    self.perforce.logger.warning(
                        "cleaning workspace to ensure have table is correctly populated. Due to lack of bless.version file in root and mismatched with previous clientname %s"
                        % prev_clientname
                    )
